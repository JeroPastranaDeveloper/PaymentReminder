...
import com.pr.servicesModule.usecase.ServicesUseCase
...
class HomeViewModel @Inject constructor(
    ...
    private fun Service.getDate(): LocalDate {
        val formatter = DateTimeFormatter.ofPattern(com.pr.paymentreminder.data.consts.Constants.DATE_FORMAT)
    ...
    private fun Service.updateDate() {
        ...
        if (this.getDate().isEqual(today) || this.getDate().isBefore(today)) {
            when (this.type) {
                com.pr.paymentreminder.data.model.PaymentType.WEEKLY.type -> this.date = this.getDate().plus(1, ChronoUnit.WEEKS).format(
                    DateTimeFormatter.ofPattern(com.pr.paymentreminder.data.consts.Constants.DATE_FORMAT))
                com.pr.paymentreminder.data.model.PaymentType.MONTHLY.type -> this.date = this.getDate().plus(1, ChronoUnit.MONTHS).format(
                    DateTimeFormatter.ofPattern(com.pr.paymentreminder.data.consts.Constants.DATE_FORMAT))
                com.pr.paymentreminder.data.model.PaymentType.YEARLY.type -> this.date = this.getDate().plus(1, ChronoUnit.YEARS).format(
                    DateTimeFormatter.ofPattern(com.pr.paymentreminder.data.consts.Constants.DATE_FORMAT))
            }
